"""Audit-Logger fuer Airbnb-OTA [CRUX-MK].
...
class AuditEntry:
    ...
    def sign_payload(payload: str, secret: Optional[str] = None) -> str:
        if secret is None:
            secret = (
                os.environ.get("DF_OTA_AIRBNB_HMAC_SECRET")
                or os.environ.get("DF_SERVICE_IDENTITY_SECRET")
                or "df-ota-airbnb-adapter-runtime-default"
            )
...
class AuditLogger:
    def __init__(self, audit_dir: str = "audit", df_id: str = "df-ota-airbnb-adapter"):
