"""Airbnb-Webhook-Handler [CRUX-MK].
...
class AirbnbWebhookHandler:
    ...
    def __init__(self, secret: Optional[str] = None, sandbox_mode: Optional[bool] = None):
        ...
        self.secret = secret or os.environ.get(
            "DF_OTA_AIRBNB_WEBHOOK_SECRET", "df-ota-airbnb-mock-webhook-secret"
        )
        ...
        try:
            ...
            if timestamp is not None:
                ...
                if age > self.REPLAY_WINDOW_S:
                    logger.warning(f"[airbnb-webhook] replay window exceeded: {age:.0f}s")
                    return False
            expected = hmac.new(
            ...
            logger.error(f"[airbnb-webhook] verify_signature failed: {e}")
            return False

    def parse_event(self, body: bytes) -> dict:
        """Parse Airbnb-Notification Body (JSON)."""
        import json
        try:
            return json.loads(body.decode("utf-8"))
        except Exception as e:
            logger.error(f"[airbnb-webhook] parse_event failed: {e}")
            return {}
