"""Airbnb-Adapter-Orchestrator [CRUX-MK].

LaunchAgent-Entry-Point fuer Welle-37 HeyLou-Mosaic-Adapter Airbnb.
...
class AirbnbAdapterOrchestrator:
    """Orchestriert Airbnb-Adapter-LaunchAgent-Run."""

    DF_ID = "df-ota-airbnb-adapter"

    def __init__(self, tenant_id: Optional[str] = None):
        self.tenant_id = tenant_id or os.environ.get("DF_OTA_AIRBNB_TENANT_ID", "hildesheim")
        ...
        self.auth = AirbnbAuthManager(sandbox_mode=self.sandbox_mode)
        self.connector = AirbnbConnector(sandbox_mode=self.sandbox_mode)
...
def main():
    ...
    orch = AirbnbAdapterOrchestrator()
    report = orch.run()
