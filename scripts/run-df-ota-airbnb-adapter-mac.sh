#!/bin/bash
# DF-OTA-Airbnb-Adapter Mac LaunchAgent Runner [CRUX-MK]
# K16 concurrent_spawn protection via mkdir-Mutex
# Welle-37

set -euo pipefail

LOCK_DIR="/tmp/df-ota-airbnb-adapter.lock"
...
if ! mkdir "$LOCK_DIR" 2>/dev/null; then
  echo "[df-ota-airbnb-adapter] K16 lock held, exiting (ok)" >&2
  exit 3
fi
echo "$$" > "$LOCK_DIR/pid"
...
report = orch.run()
print(f'[df-ota-airbnb-adapter] loop_id={report.loop_id} status={report.final_status} sandbox={report.sandbox_mode}')
